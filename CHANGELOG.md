...
## [5.0.0] — 哪天对你好坏、穿什么颜色、往哪个方位，都给明确答案

### 个人日子吉凶：《协纪辨方书》卷三十三「相主」

此前问「这周哪天对我好」只能得到「本库现有条款里没有这一类」。协纪卷三十三正好写了择日怎样看人：「相主者何以四柱八字輔相主人之命也從來皆論生年不論生日有論生日者非古法也」（`xieji:c033:p0020`）——看的是本人出生那一年的干支，不是日主。本版按这一段实现（`scripts/xiangzhu.py`），每条规则都是这段的原话，测试逐字核对：

- **凶**：天干克、地支冲本命为「天尅地衝最凶」（大凶）；天比地冲与附表六十条纳音克冲为「選擇家通忌」；寅卯、巳午年生被冲为「凶莫堪」；七杀两点「必凶」；劫财、枭印、食伤、羊刃三点「多見則忌」。辰戌丑未、申酉、亥子年生被冲，原文说「略輕」「止主是非」，定为小凶；但由太岁来冲仍是凶（「然太嵗衝之亦凶」）。
- **吉**：命禄、命贵人、食禄「最吉」，合官「貴格」、合财「富格」、干支都合「上上格」，定为大吉；比肩、正印、长生、驿马、六合、三合、财官一点定为吉。凶的条目压过吉的：原文以「不衝命尅命」为吉课前提。七杀只有一点时，年月不利为平，否则至多为吉（「或年月利而干係七煞一㸃可也」）。
- 年、月、日分开报（「太歳衝命最凶月次之日又次之」）；日等级只看当天的干支，按「点」计数的条目才把三柱合起来算。
- 禄、驿马、长生、羊刃、贵人各有出处表；长生只用于阳干（原文例子是壬申、丙寅），羊刃只有阳干（三命通会「陰干無刃」）。天乙贵人两说（三命通会「甲戊庚牛羊」、渊海子平「庚辛逢马虎」）只在庚干上不同：庚年生人的丑未午寅日两说并列，不计入吉凶。本命日按协纪「今人忌本日何歟」取比肩，《选择要略》忌本命日的说法写在层 2。
- 原文的例子是修造（以宅长之命为主）和安葬（以亡命为主），天克地冲、天比地冲写明是选择家对一切用事的通忌；用于其他事情时回答会说明。卷三十三里「补龙扶山」看房屋坐山，没有实现。`references/forecast-source-audit.json` 里这一段的用途由「只作范围审计」改为「相主按生年」，并写明以上限度。

**用在哪里**

- 问一段时间的运势：新状态 `personal_calendar`，逐日（超过一个月按节气月）给出等级。首句例如「按你出生那年的干支（丁丑）看，这段时间对你最好的日子是10月14日（辛酉，财官、命贵人、三合）、10月15日（壬戌，合官）……都是大吉；要避开10月24日（辛未，纳音克冲）」。
- 比较候选档期：事项忌日先排除；相主再排除对本人凶、大凶的日子（排除项带 `participant_id`、`birth_year`）；其余按大吉、吉、平、小凶分 tier 1–4。为两人同等安排时取对较差那人的等级。实际选择先取最好的 tier，「尽早」「尽晚」「候选顺序」只在同一 tier 里排序；只有最好的那层里有几个选项又没给偏好时，才请用户选。
- 更吉的候选因为整段碰到忌时而排不进去时，首句点名它和那段时辰（`practical_choice.passed_over`），不再只说「同等的再按你的偏好」。
- 相主对所有择时事项都适用，排序流程因此对面试、自定义事项也会运行，截路空亡的忌时（原文「以至百事…皆不利」）随之生效；以前这些事项不做任何筛选。
- 生在立春前后又缺时刻时，年柱有两种可能（如己卯或庚辰）：两种都算，每天取较差的那种——两种都好才算好日子，任一种凶就避开，首句写明「己卯或庚辰，生在立春前后，按较差的算」。连候选都没有时出 `birth_year_required`，首句说明缺出生时刻。
- 裁决表升为 `precedence-v3`，新增「个人相主」一节；`capabilities` 里出行等择时与期间运势的 `personal_ranking` 改为 `rule_based`。

**结果会变的例子**：2026-09-21 与 22 日出发、偏好「尽早」、丁丑年生：以前首选 21 日；现在首选 22 日（己亥，命贵人、驿马，大吉），21 日（戊戌，平）为备选。「尽早」只在同一等级里排序，这里大吉只有 22 日一个。

### 穿什么颜色、戴什么

问颜色或佩戴时（`bazi_reading.py --question … --markdown`，或 `fortune_reading` 带这类问题），首句直接回答，例如「按你的八字，衣服首选红色、紫色（火），其次绿色、青色（木）；少穿黑色、深蓝色（水）。佩戴也按这个次序……」。依据只用调候一法（`scripts/wuxing_colours.py`）：

- 《穷通宝鉴》本日主本月令一段按原文先后取的用神，引的是写到这个用神的那一句（如「十一月庚金，天气严寒，仍取丁甲，次取丙火照暖」）；
- 天干换五行，五行按《三命通会》卷七「其色青…其色赤…其色黄…其色白…其色黑」换颜色，色名参《梅花易数》卷二「青碧綠色屬木，紅紫赤色屬火」，饰物按《梅花易数》卷三物象；
- 少穿的颜色取克首选用神的那一行（子平「命中所忌，我逆而施之者」），这一行若本身也在原文所取之列就不列；
- 原文另讲的条件写在层 2；登记表备注里「不作现实建议」一类用法说明不再引到颜色回答里，原文条件照留。古籍讲的是八字需要哪种五行，没说穿颜色能改变遭遇，回答里说明一次。日主或月令未定时说明原因。

120 个日主月令格子全部由测试跑过：引文都是原文、都写到所取的天干，少穿的颜色从不是原文首要所取的五行（原文按条件另取的，写在层 2 的条件里）。

### 方位、幸运数字、五行缺什么

同一条调候用神还回答三类常见问题，问什么答什么，可以组合（「幸运色和幸运数字」）：

- **有利方位、幸运数字**：五行按《协纪辨方书》卷一「河圖一六為水居北二七為火居南三八為木居東四九為金居西五十為土居中」（`xieji:c001:p0017`）换成方位与数字，例如「有利方位是南方（火），其次西方（金）；少往北方（水）」「幸运数字是2、7（火），其次4、9（金）」。问家具、房间朝向的归风水专项，不用这里的方位。
- **五行缺什么、喜什么五行**：首句「按《穷通宝鉴》调候，你的八字最需要火（丁），其次金（庚）」；层 2 说明古法不数哪种五行少，而是看最需要哪种，《子平真诠》按格局取用神是另一种方法，不合在一起。

### 带事项的日子：下个月哪天搬家好

期间请求带事项（`intent: period`，`event.scenario` 为出行、婚嫁、搬家、开业）时，逐日先套这件事的忌日，再按相主分吉凶：「这段时间搬家对你最好的日子是10月23日（庚午，财官、命禄、食禄）……；搬家要避开10月21日（戊辰，月破、往亡，忌搬家）、10月29日（丙子，歸忌，忌搬家）；另外要避开10月24日（辛未，纳音克冲）」。问单独一天（「10月29日搬家可以吗」）时首句先答「不行。」并说是哪条忌日，再说这天对本人的等级。原话问「哪天」时即使跨度超过一个月也逐日给出（最长一年，第二层只列最好和要避开的日子）。

### 问法路由

新增 `scripts/question_router.py`：给一句原话，返回该走的流程（`personal_days`、`event_slots`、`wear_advice`、`almanac`、`natal`、`specialist`、`boundary`）、命令、请求骨架和还缺的资料，只做路由、不算命盘。SKILL.md 加了问法对照表与触发关键词（哪天对我好、吉日、今天运气、个人择日、幸运色、佩戴、幸运数字、有利方位、五行缺什么）。测试用四十多条真实问法逐条核对流程、事项和时间词，并有两条从路由一路算到回答首句的端到端检查；路由会给出的每个时间词都由测试确认计算端认得。时间词新增后天、这周、这个星期、下个星期、周末、这个月、下月、最近一周。

### 其他

- 问句分类加入「好不好」「吉不吉」「宜不宜」「顺不顺」，按是非问回答。

**输出形态变化**：新增 `personal_calendar`（含 `event_hits`）、`colour_advice`（含 `aspects`）、`question_router.py`，排序行的 `personal` 与多级 tier，`practical_choice.passed_over`、`Placement.grade`，能力声明与裁决表版本改变，择时首选可能与旧版不同，故取主版本号。

## [4.9.0] — 黄历、六爻能直接回答；缺地点时不再说「不建议」

### 修正

- **缺活动地点经度时，择时回答说「不建议」，`--markdown` 还会崩溃。** 真太阳时没有事件经度就只算到年、月柱，窗口没有日柱、忌日条款没法套用。实际选择却把这种情况记成 `clause_conflict`，首句成了「不建议。……目前不能推荐」，像是条款排除了它；这行数据还缺 `end`，白话输出直接抛 `KeyError`。现在新增状态 `screening_incomplete`，首句是「现在判断不了，因为还不知道活动地点：日柱按当地真太阳时定，缺经度就定不了……告诉我在哪个城市或经度，就能接着核。」没算到日柱的行补上 `end`。
- **同一张盘在不同进程里 JSON 不同。** 子卯互刑的两个地支取自 frozenset，输出顺序随 Python 每个进程的字符串哈希变化，时而「卯、子」时而「子、卯」。改为按原文「子刑卯」的固定顺序。新增 `tests/test_determinism.py`：八字、择时（JSON 与白话）、黄历各在五个 `PYTHONHASHSEED` 下运行，输出必须逐字节相同；用旧代码复跑，这条测试会失败。

### 黄历、六爻的白话输出

- `huangli_query.py --markdown`，可带 `--question`。问到出行、结婚、搬家、开业时，首句按问法直接回答（「不行。」「已查条款不忌。」），依据天地转杀与协纪吉神化解不了的忌日；通书宜忌表与条款不一致时在同一句里指出。没问具体事时首句给当日宜忌，并点出其中被已核条款列为忌的项目。问到这四件以外的事，首句说明它不在已核条款范围里，只能看通书表。JSON 新增 `day_prohibitions`：四件事各自当天命中的条款。
- `liuyao_cast.py --markdown`：首句说本卦、动爻和变卦，再说用神取哪一类、出处（《增删卜易·用神章》或「本库尚未核到古籍出处」）以及它在卦中的位置、旺衰、世应、空破。工具不把旺衰、日月生克和动变合成吉凶，所以问「能不能成」时明说卦上没有现成断语，不替用户下结论。
- 首句的时间改为当地钟面写法：日期与 UTC 偏移只写一次（「2026-09-21 08:00–10:00（Australia/Sydney，UTC+10）」），跨夏令时切换时两端各注偏移。

### 首句用词规则由代码统一检查

`references/27-direct-answer.md` 的甲类禁用词、乙类限位词移入 `scripts/answer_style.py`，由测试与文档逐字核对。此前只有择时回答受检；现在八字、黄历、六爻、择时四种白话输出都按同一份词表扫描。扫描当场发现黄历在天转日崩溃（条款冲突项没有白话句），已修正。

### 结构

- `calculate_bazi`（376 行）拆为出生地、公历基准日、历法、四柱、司令、神煞格局等、流年、交节背景几个阶段；`read_request`（199 行）拆为整段事件、补期间、优先对象、骨架、逐人计算、候选筛查、实际选择几个阶段。重构前后用 533 个择时请求与 2023 组排盘参数（取自测试集运行时的真实输入，另加四类事项 × 日期 × 偏好 × 问法的网格）比较，JSON（含键序）与白话文本逐字节相同。
- 新增 `scripts/contracts.py`：忌日命中、排序行、实际选择、推荐状态、阻断项的 TypedDict。生产函数以它们为返回类型，mypy 由此查出一处未收窄的 `first_choice` 取值；`tests/test_contracts.py` 用真实输出逐键核对，多键、缺键都失败。

**输出形态变化**：新增推荐状态 `screening_incomplete`、黄历 `day_prohibitions` 与两个 `--markdown`，故取 minor 版本号。

## [4.8.0] — 婚嫁、搬家、开业有了有出处的忌日筛选

### 《协纪辨方书》里吉神化解不了的忌日

此前只有出行一项做条款筛选，而且只有一条日级条款（天地转杀）。本版把《协纪辨方书》卷十里自己写明「與併猶忌」「德神不能化解」的几条接进来：月破、四废、四忌、四穷、往亡、归忌。每条都带三段原文：起例（怎么算出这一天）、所忌（忌哪些事）、为什么吉神也化解不了。

- 一条适用哪件事，只看它的所忌原文有没有列出该事项：出行作「行幸」、搬家作「般移」，依据是卷十一的注「出行同」「移徙同」；婚嫁作「嫁娶」，开业作「開市」。所以往亡不管开业，归忌只管搬家，四忌只管婚嫁。卷十一逐事列出的忌项与此完全一致，由测试核对。
- 婚嫁、搬家、开业由此从「未实现」变为有通用忌日筛选（`calendar_screening: rule_based`）；个人吉凶排名仍未实现。天地转杀只筛出行与嫁娶，因为原文只点了这两项。
- 协纪多数日子要先把吉神凶煞「分為六等」权衡再定宜忌，结果在卷二十一至三十二的月表里，而本库转录只有表头。所以本版不实现任何「宜」，也不实现要经权衡的「忌」；回答里会明说「没碰到这几种日子，不等于这天宜……」。
- 独立核对：十年间逐日与 lunar_python 自带的神煞表比较，只在交节当天（它把整天算入新月）和一个表格格子（亥月己丑的归忌）上不同，后者起例并无例外，按原书处理并在测试里单独登记。
- 裁决表版本改为 `precedence-v2`。

### 黄历标出与原书相反的宜项

`clause_conflicts` 现在也列出通书表记为「宜」、而协纪写明吉神化解不了的事项。例如 2020-06-02 丙子是夏季四忌，协纪忌嫁娶，通书表却列嫁娶为宜；协纪自言「舊本無四忌今依起例補之」。只并列，不裁决。

### 选时更准

- 截路空亡按「以日取时」：晚子时（23:00–24:00）的时干来自次日五鼠遁，所以按次日的规则判断。壬戌日夜子是壬子、属癸日一组，戊戌日夜子是甲子、不在两说之内。此前按当天判断，戊、癸日的夜子被误标，丁、壬日的夜子被漏掉。
- 实际选择只在「干净的开始时间」里进行：从每个窗口里去掉会让整件事碰到忌时的开始时刻。「尽早」如果窗口开头就碰到忌时，会顺延到忌时之后，而不是直接报冲突；没给偏好时只给干净区间，有几段就列出来请用户选。所有窗口都找不到干净的开始时间才算冲突，并说明放不下完整时长。

**输出形态变化**：新增 `xieji_days` 规则、`rule_stem`、排除项的 `sources`/`plain`、`alternatives` 带干净区间；婚嫁、搬家、开业的能力声明改变，裁决表版本升级，故取 minor 版本号。
