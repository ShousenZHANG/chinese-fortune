...
5. 纯本命默认只调用 `scripts/bazi_reading.py`：`chart_facts` 给盘面，`rule_assessment` 给相关条件，`evidence_bundle` 给完整原文与例外。未来时段按下方专用路由；首次需要本命解读时带 `include_natal_reading:true`。两条路径都不为找 `ge_ju` 或 `yong_shen` 再跑诊断。已有段落不重复查询，缺本题条款才用 `classical_search.py --query` 或 `--passage-id` 补查。
