...
截至本轮扩充，`knowledge/` 收录13部书的所选转录目录：535个章节或卷单元、18,982个段落。原有五书416单元、8,385段保持原ID与文本不变。这里的“完整”仅指下表网络转录版本的目录已收齐，不指所有版本汇编，不指原刻逐页校勘完成，也不代表其中命理断语已经得到经验验证。
...
| ID | 所选来源 | 单元数 | 版本与使用边界 |
|---|---|---:|---|
| xieji | 协纪辨方书四库本卷01–36 | 36 | 全36卷转录；卷首总序与目录保存在来源索引，不冒充已校影像 |
| xuanze | 选择要略上中下 | 3 | 朝鲜典籍，具体印本未确认；事项规则不能自动变成个人八字排序 |
| ziwei | 紫微斗数全书网络三卷选本 | 3 | 不能称其他多卷版本全文；原有4条基础规则不因此扩大 |
| meihua | 梅花易数网络三卷选本 | 3 | 不能称其他五卷版本全文；题署按源保留 |
| liuren | 六壬大全四库本卷01–12 | 12 | 与主算法规格的四库来源一致，仍须区分不同课法 |
| dunjia | 遁甲演义四库本卷1–4 | 4 | 未选普通网页中卷界不全的候选；置闰法不冒充当前拆补口径 |
| zengshan | 增删卜易网络四卷整理本 | 34 | 卷一31单元加卷二至四整卷；排除明确现代增订，不等于已还原古刻 |
| yuanling | 奇门遁甲元灵经网络24卷 | 24 | revision 8259389，与现有算法规格所引版本一致；完整算法另计 |

共新增119单元、10,597段。原文按固定MediaWiki revision保存，原始wikitext及索引进入来源包，运行包只带检索正文及出处。新增普通正文保守标记为历史作品转录；显式注文分层，无法解析的模板、图示和缺字仍保留标记，不擅自补字。增删卷一已排除显式“原剑按”，卷首现代录入说明不进入运行正文；源文中未标记的改写仍待版本校勘。

这些网络材料未做全书影像校勘。现有五条子平短句影像见证仍仅适用于对应短句与所列异版，不把新库或整部原书升级为已校。

维护者可用 `import_classics.py --supplements-from <捕获批次目录>` 从显式API捕获重建；目录、revision、时间戳和源文本必须完整。源码保存的每卷来源与目录能够重建该批次，测试核对重建正文一致性。普通使用不联网更新。

按场景查阅见 [古籍适配](../references/25-classical-research.md)。八类取运包保留全章，个人条件仍需当次核查；16类入口不代表16类预测全已实现。
...
顶层 `library_id=chinese-classics-v2`，清单 `schema_version=2.0`，显式声明 `distribution_kind=source/runtime`。书级信息包括版本、来源、revision 或片段 SHA-256、预期目录、逐章路径/哈希、授权与质量状态。章节另保留来源档案内的原始路径/哈希和段落哈希，既有段落格式没有随发行清单改变。
...
1. 清单要求的全部书目存在，预期目录非空且无重复；获取章节集合必须与预期集合相等。
...
构建先验证完整源码目录闭包，再把 HTML/wiki 等原始来源文件移入独立 sources ZIP，运行包保留十三书全部选定章节及来源索引。运行清单逐一校验运行文件和来源档案索引；原始路径明确属于 source_archive/knowledge，不假装文件在运行包内。外部 SHA256SUMS 覆盖两包，清单不做循环自哈希。源码缺来源失败，运行包缺正文失败，目录消失不能自动切换较弱模式。
