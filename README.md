...
- **历法严谨** — 真太阳时、节气定月、立春年界、夜子时、闰月等业余易错处全部正确，并经**独立引擎 sxtwl 跨库对照**，分两层：(1) **本仓库引擎**端到端逐盘比对 1920–2080 每 29 天 × 3 个时辰（含夜子/早子两侧）共 6,045 盘，零分歧；(2) 底层历法库 `lunar_python` 对 sxtwl 逐日全量比对 58,440 天，零分歧——这一层校验的是依赖库，不是本仓库代码，两者分开陈述。
